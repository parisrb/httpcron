...
- install the bundler gem if you don’t have it (+gem install bundler+)
...
- in the application directory run +rake assets:build+ to generate the optimiezd assets
- in the application directory run +rackup+
...
- # this value can be configured in the /config.rb file
...
- limit the max number of records to return, default and max value is 100#
- offset index of the first item to be returned
...
Get the execution with the corresponding id, non-admin users can only access the executions of their own tasks

  {
    "id":1,
    "status": 200,
    "start_at": "2011-08-16T16:56:22+00:00",
    "url":"http://localhost:9292/",
